...
buyOneMore (MkShoppingEntry x y count) = MkShoppingEntry x y (count + 1)
...
toggle (Decreasing val) = Increasing val
...
rgb (Mix c1 c2) = [(r1 + r2) / 2, (g1 + g2) / 2, (b1 + b2) / 2]
 where
  [r1, g1, b1] = rgb c1
  [r2, g2, b2] = rgb c2
rgb (Invert c1) = [1 - r1, 1 - g1, 1 - b1] where [r1, g1, b1] = rgb c1
...
data KeyVals k v = Empty | Pair k v (KeyVals k v) -- {key:: k, value:: v, rest:: KeyVals k v}
  deriving Show
...
fromList [] = Empty
-- fromList ((k, v):xs)= Pair {key=k, value= v, rest = fromList xs}
...
toNat n
  | n < 0 = Nothing
  | n == 0 = Just Zero
  | otherwise = Just (PlusOne y)
  where (Just y) = toNat (n-1)
