...
import           Data.List
import           Data.List.NonEmpty             ( NonEmpty((:|)), reverse, nonEmpty, last )
import           Data.Monoid
...
member _  (Set []      ) = False
member el (Set (x : xs)) = el == x || member el (Set xs)

-- add a member to a set
add :: (Eq a, Ord a) => a -> Set a -> Set a
-- add el emptySet = Set [el]
add el (Set xs) | member el (Set xs) = Set xs
                | otherwise          = Set (sort (el : xs))
...
data State = Start | WithEggs | WithFlour | WithSugar | WithSugarAndFlower| DoneMixing | Error | Finished
  deriving (Eq,Show)
...
step WithFlour          AddSugar = WithSugarAndFlower
step WithSugar          AddFlour = WithSugarAndFlower
step WithFlour          _        = Error
step WithSugar          _        = Error
step WithSugarAndFlower Mix      = DoneMixing
step WithSugarAndFlower _        = Error
step DoneMixing         Bake     = Finished
step DoneMixing         _        = Error
step Error              _        = Error
...
reverseNonEmpty (x:|xs) = y where (Just y) = nonEmpty (Data.List.reverse xs ++ [x])
...
instance (Ord a)=> Semigroup (Set a) where
  (<>) (Set []) (Set b) = Set b
  (<>) (Set (x:xs)) (Set b) = Set xs <> add x (Set b)
...
passwordAllowed pass (ContainsSome str) = any (`elem` str) pass
passwordAllowed pass (DoesNotContain str) = all (`notElem` str) pass
...
evaluate (Operation "+" arith1 arith2) = evaluate arith1 + evaluate arith2
evaluate (Operation "*" arith1 arith2) = evaluate arith1 * evaluate arith2
