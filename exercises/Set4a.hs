...
compareFun x y | length x > length y                     = x
               | length x == length y && head x < head y = x
               | otherwise                               = y

longest :: (Eq a, Ord a) => [[a]] -> [a]
longest = foldr compareFun []
...
incrementKey k = map (\(key, v) -> if key == k then (key, v + 1) else (key, v))
...
winner scores player1 player2
  | Map.findWithDefault 0 player1 scores >= Map.findWithDefault 0 player2 scores
  = player1
  | otherwise
  = player2
...
freqsAdd :: Maybe Int -> Maybe Int
freqsAdd (Just x) = Just (x + 1)
freqsAdd Nothing  = Just 1

freqs :: (Eq a, Ord a) => [a] -> Map.Map a Int
freqs = foldr (Map.alter freqsAdd) Map.empty
...
transfer from to amount bank
  ...
  | y < amount = bank
  | amount < 0 = bank
  | otherwise = Map.adjust (\x -> x - amount)
                           from
                           (Map.adjust (+ amount) to bank)
  where (Just y) = Map.lookup from bank
------------------------------------------------------------------------------
...
maxIndex arr = foldr (\(i, e) maxIdxSoFar -> if e > arr ! maxIdxSoFar then i else maxIdxSoFar) (head (indices arr)) (assocs arr)
